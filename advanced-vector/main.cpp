...
void Test6() {
    ...
    }
    {
        ...
        assert(Obj::num_copied == 1);
        assert(Obj::num_default_constructed == SIZE);
       // cout << Obj::GetAliveObjectCount() << endl;
    ...
    /*    cout << std::count_if(v.begin(), v.end(), [](const TestObj& obj) {
            return obj.IsAlive();
        }) << endl; */
        size_t ch = 0;
        cout << "size = " << v.Size() << endl;
        assert(std::all_of(v.begin(), v.end(), [&ch](const TestObj& obj) {
            ch += obj.IsAlive();
            cout << ch << endl;
            return obj.IsAlive();
        }));
